...
<body>
    <div class="email-title">
        <p class="email-title__greet-first">Chào bạn {{$data['ten_khach_hang']}}!</p>
        ...
        <div class="email-title__info-recieve">
            ...
            <p class="info-recieve__inf">Họ Tên: <span>{{$data['ten_khach_hang']}}</span></p>
            <p class="info-recieve__inf">Số điện thoại: <span>{{$data['sdt']}}</span></p>
            <p class="info-recieve__inf">Địa chỉ: <span>{{$data['dia_diem']}}</span></p>
        </div>
    </div>
    <div class="email-content">
        <p class="info-order__title">Mã đơn hàng: <span>{{$data['id_don_hang']}}</span></p>
    ...
    <table>
        ...
        @foreach($data['ds_gio_hang']->dsNuocHoa as $gio_hang)
        <tr class="email-content__product">
            <td class="email-content__product-name">{{$gio_hang['thongTinNuocHoa']->ten_nuoc_hoa}}</td>
            <td>{{number_format($gio_hang['thongTinNuocHoa']->gia_tien)}}vnđ</td>
            <td>{{$gio_hang['soLuong']}}</td>
            <td>{{number_format($gio_hang['giaKhuyenMai'])}}vnđ</td>
            <td>{{number_format($gio_hang['giaTien'])}}vnđ</td>
        </tr>
        @endforeach
    </table>
    <div class="email-title__info-recieve">
        <p class="info-recieve__inf">Tổng tiền: <span>{{number_format($data['tong_tien'])}}vnđ</span></p>
        <p class="info-recieve__inf">Tổng khuyến mãi: <span>{{number_format($data['tong_tien_giam'])}}vnđ</span></p>
        <p class="info-recieve__inf">Tổng số lượng: <span>{{number_format($data['tong_so_luong'])}}</span></p>
        <p class="info-recieve__inf">Tổng thanh toán: <span>{{number_format($data['tong_thanh_toan'])}}vnđ</span></p>
    </div>
