...
<div class="content-admin content-admin__ckeditor">
    <div class="grid_g">
        <div class="add-form__header add-form__add-account--center add-form__haeder-product">
            <h3 class="add-form__haeding add-form__haeding-product">Cập Nhật Slideshow</h3>
        </div>
        <form action="{{route('quanly.cap-nhat-slideshow', $timSlideShow['id'])}}" method="POST" id="cap-nhat-slideshow" data-parsley-validate enctype="multipart/form-data">
            @csrf
            <div class="row_g">
                        ...
                        <div class="add-form__form">
                            ...
                            <div class="add-form__group">
                                ...
                                <div class="add-form__item">
                                    <input type="text" class="add-form__input add-form__input-slideshow" id="ten_slideshow" name="ten_slideshow" value="{{$timSlideShow['ten']}}"
                                    placeholder="Tên slideshow" required data-parsley-required-message="Vui lòng nhập tên slideshow!">
                                </div>
                            </div>
                            ...
                            <div class="add-form__group">
                                ...
                                <div class="add-form__item">
                                    <textarea class="add-form__input add-form__textarea-slideshow" id="mo_ta_slideshow">{{$timSlideShow['mo_ta']}}</textarea>
                                </div>
                            </div>
                    ...
                    <div class="row_g row__banner" id="row-0-panel">
                        <div class="col_g l_12_g event-rght__ip">
                            ...
                            <div class="event-right__content" style="text-align: left;">
                                <label id="id-label-0" for="event__input-0" class="form-control form-control__slideshow event-rght__label ">{{$ten_file}}</label>
                                <input hidden class="form-control event-rght__input" id="event__input-0" name="duong_dan_banner" type="file" multiple placeholder="Nhập đường link"
                                ...
                                <img class="event-right__img" id="event__img-0" src="../{{$timSlideShow['duong_dan']}}" alt="slider" width="100%" height="320px">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="add-form__group">
                            ...
                            <div class="add-form__item">
                                <select class="add-form__input add-form__select-status" id="trang_thai_slideshow" required data-parsley-required-message="Vui lòng chọn chọn trạng thái!">
                                    <option value="0" {{$timSlideShow->trang_thai == 0 ? 'selected' : ''}}>Ẩn slider</option>
                                    <option value="1" {{$timSlideShow->trang_thai == 1 ? 'selected' : ''}}>Hiện slider</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
<!-- Cập nhật slideshow -->
<script>
    $('#cap-nhat-slideshow').submit(function(event) {
        event.preventDefault();
        if ($(this).parsley().isValid()) {
            var form = $(this);
            var url = form.attr('action');
            let add_slideshow = document.getElementsByName('duong_dan_banner');

            // Khai báo formData
            var formData = new FormData($(this)[0]);
            if (add_slideshow[0].files[0] != undefined) {
                formData.append('data_add_slideshow', add_slideshow[0].files[0]);
            }
