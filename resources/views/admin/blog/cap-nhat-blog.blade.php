...
<div class="content-admin content-admin__ckeditor">
    <div class="grid_g">
        <div class="add-form__header add-form__add-account--center add-form__haeder-product">
            <h3 class="add-form__haeding add-form__haeding-product">Cập Nhật Trang Tĩnh</h3>
        </div>
        <form action="{{route('quanly.cap-nhat-blog',$searchBlog['id'])}}" method="POST" id="cap_nhat_blog" data-parsley-validate enctype="multipart/form-data">
            @csrf
            <div class="row_g">
                        ...
                        <div class="add-form__form">
                            <div class="add-form__group">
                                ...
                                <div class="add-form__item">
                                    ...
                                    placeholder="Tiêu đề" required data-parsley-required-message="Vui lòng nhập tiêu đề!" style="height: 64px;">{{$searchBlog['tieu_de']}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="add-form__form">
                            <div class="add-form__group">
                                ...
                                <div class="add-form__item">
                                    ...
                                    placeholder="Mô tả" required data-parsley-required-message="Vui lòng nhập mô tả!" style="height: 64px;">{{$searchBlog['mo_ta_noi_dung']}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row_g row__banner" id="row-0-panel">
                        <div class="col_g l_12_g event-rght__ip">
                            ...
                            <div class="event-right__content" style="text-align: left;">
                                <label id="id-label-0" for="event__input-0" class="form-control form-control__slideshow event-rght__label">{{$nameFile}}</label>
                                <input hidden class="form-control event-rght__input" id="event__input-0" name="duong_dan_banner" type="file" multiple placeholder="Nhập đường link" data-parsley-length="[0, 150]"  data-parsley-length-message="Tên file tối đa 150 kí tự!" onchange="uploadBannerFile(this, 0)" accept=".jpg, .png">
                                <img class="event-right__img" id="event__img-0" src="../{{$searchBlog['duong_dan']}}" alt="blog" width="450px" height="320px">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="add-form__group">
                            ...
                            <div class="add-form__item">
                                <select class="add-form__input add-form__select-status" id="trang_thai_blog" required data-parsley-required-message="Vui lòng chọn chọn trạng thái!">
                                    <option value="1" {{$searchBlog->trang_thai == 1 ? 'selected' : ''}}>Hiện trang tĩnh</option>
                                    <option value="0" {{$searchBlog->trang_thai == 0 ? 'selected' : ''}}>Ẩn trang tĩnh</option>
                                </select>
                            </div>
                        </div>
                    </div>
            ...
            <label class="title__news">Nội dung:</label>
                  <div class="add-form__group add-form__group-add-news">
                    <div class="add-form__item">
                      <textarea type="text" id="ckeditor_blog" name="ckeditor_blog" class="add-form__input">{{$searchBlog['noi_dung']}}</textarea>
                    </div>
                  </div>
            ...
            <div class="row_g row__add-product">
                <div class="col_g l_12_g">
                    <div class="add-form__controls add-form__controls-add-product">
                        <a href="{{route('quanly.ql-blog')}}" class="btn add-form__controls-back btn--normal btn--des-account">HỦY</a>
...
<!-- Cập nhật Blog --> 
<script>
    $('#cap_nhat_blog').submit(function(e) {
        e.preventDefault();
        if ($(this).parsley().isValid()) {
            var form = $(this);
            var url = form.attr('action');
            let add_blog = document.getElementsByName('duong_dan_banner');
            ...
            if(add_blog[0].files[0] !=  undefined) {
                formData.append('data_blog', add_blog[0].files[0]);
            }
